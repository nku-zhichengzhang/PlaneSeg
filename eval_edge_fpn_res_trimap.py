...
from trimaputil import evaluateTrimap
    ...
    for name, detector in detectors:
        ...
        for sampleIndex, sample in enumerate(data_iterator):
            ...
            for c in range(len(detection_pair)):
                errors_of_union, errors_of_gt, errors_of_pred = evaluateTrimap(detection_pair[c]['masks'][:, 80:560], input_pair[c]['masks'][:, 80:560])
                all_statistics.append([errors_of_union, errors_of_gt, errors_of_pred])
                continue

    np.save(options.test_dir + '/edge_fpn_res_trimap.npy', all_statistics)
