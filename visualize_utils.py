...
#  Batch visualization
############################################################
def visualizeBatchDeMoN(options, input_dict, results, indexOffset='', prefix='', concise=False):
...
def visualizeBatchPair(options, config, inp_pair, detection_pair, indexOffset='', prefix='', suffix='', write_ply=False, write_new_view=False):
    detection_images = []    
...
def visualizeBatchRefinement(options, config, input_dict, results, indexOffset='', prefix='', suffix='', concise=False):
    if not concise:
...
def visualizeBatchDetection(options, config, input_dict, detection_dict, indexOffset=0, prefix='', suffix='', prediction_suffix='', write_ply=False, write_new_view=False):
    ...
    if 'depth' in input_dict:
        ...
        depth_gt = depths[0]
        savedpath = options.test_dir + '/' + str(indexOffset) + '_depth_gt' + suffix + '.png'
        disp_to_img = resize(depth_gt, output_shape=(480, 640))
        plt.imsave(savedpath, disp_to_img, cmap='plasma')
        # cv2.imwrite(options.test_dir + '/' + str(indexOffset) + '_depth_gt' + suffix + '.png', drawDepthImage(depth_gt[80:560]))
        pass
    ...
    if 'depth' in detection_dict:    
        ...
        np.save(options.test_dir + '/' + str(indexOffset) + '_depth' + suffix + prediction_suffix + '.npy', depth_pred[80:560])
    ...
    if write_ply:
        ...
        pose_filename = current_dir + '/' + options.test_dir + '/'+ '/pose_new_view.txt'
        print(pose_filename)
...
def drawNewViewDepth(depth_filename, masks, XYZs, camera, pose):
    drawNewViewDepth.ply_model_index += 1
    faces = []
    ...
    filename = 'test/models/depth_model'+str(drawNewViewDepth.ply_model_index)+'.ply'
    with open(filename, 'w') as f:
        header = """ply
format ascii 1.0"""
        header += imageFilename
...
"""
    ...
    try:
        ...
        pass
    return
drawNewViewDepth.ply_model_index = 0
