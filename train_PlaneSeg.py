...
def edge_loss(preds_edge, target):
    pad_h = 80
    h, w = target.shape  # target[0].size(1), target[0].size(2)
    target = torch.from_numpy(target).long()
    # np.save('checkdata/edge.npy', preds_edge.cpu().detach().numpy())
    # np.save('checkdata/target.npy', target)
    valid_target = target[pad_h: h - pad_h, :]
    pos_num = torch.sum(valid_target == 1, dtype=torch.float)
    neg_num = torch.sum(valid_target == 0, dtype=torch.float)
    ...
    valid_edge = scale_pred[:, :, pad_h: h - pad_h, :]

    valid_edge = valid_edge.cuda()
    valid_target = valid_target.unsqueeze(0).cuda()
    # print(valid_edge.size(), valid_target.size())
    e_loss = F.cross_entropy(valid_edge, valid_target, weights.cuda(), ignore_index=255)

    return e_loss


def generate_edge(label_seg, edge_width=3):
    label = label_seg[0]
...
def train(options):
    ...
    model = MaskRCNN_edge_fpn_resolution(config)
    ...
    for epoch in range(options.numEpochs):
        ...
        for sampleIndex, sample in enumerate(data_iterator):
            ...
            for c in range(1, 2):
                ...
                continue
            # print(type(gt_segmentation), gt_segmentation.shape)
            target = generate_edge(gt_segmentation)
            losses += [edge_loss(edge, target)]
