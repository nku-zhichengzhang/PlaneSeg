...
class PlaneDatasetSingle(Dataset):
    def __init__(self, options, config, split, random=True, loadNeighborImage=False, load_semantics=False, load_boundary=False):
        ...
        print(len(planenet_scene_ids_val))
        ...
        with open(self.dataFolder + '/ScanNet/Tasks/Benchmark/scannetv1_' + split + '.txt') as f:
            for line in f:
                ...
                # if split == 'test':
                #     with open('get_scene_id_test.txt', 'a') as f:
                #         f.write('%s' % scene_id + '\n')
                # print(scene_id)
        ...
        if random:
            ...
            np.random.seed(0)
            pass
        # self.sceneImageIndices = self.get_clear_indices()
    ...
    def get_test_indices():
        indice_path = './new_selected_sceneImageIndices_test.npy'
        test_indices = np.load(indice_path)
