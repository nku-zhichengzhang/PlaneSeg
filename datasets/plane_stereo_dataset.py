...
class PlaneDataset(PlaneDatasetSingle):
    def __init__(self, options, config, split, random=True, image_only=False, load_semantics=False, load_boundary=False, write_invalid_indices=False):
        ...
        if split == 'test':
            self.sceneImageIndices = self.get_test_indices()
        return
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            scene = self.scenes[sceneIndex]
            # print(np.max(self.sceneImageIndices[:,0]))
            ...
            try:
                ...
                # print(sceneIndex, imageIndex, imageIndex_2)
                # with open('test_indices.txt', 'a') as f:
                #     f.write('%d %d %d\n' % (sceneIndex, imageIndex, imageIndex_2))
            except:
                continue
